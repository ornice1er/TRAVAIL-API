...
class HsupRepository
{
    ...
    public function getAll($request)
    {
        ...
        $req = Hsup::ignoreRequest(['per_page','pageSize','page'])
     ...
     */
    public function makeStore($data): Hsup
    {
        $model = new Hsup($data);
        /*$aws= new AwsService();
        if(request()->file('file'))  $model->file = $aws->upload(request()->file('file'),"Hsups")['full_url'];*/
        $model->save();
     ...
     * Met à jour une fête.
     */
    public function makeUpdate($id, $data): Hsup
    {
        $model = Hsup::findOrFail($id);
        /*$aws= new AwsService();
        if(request()->file('file'))  $data['file'] = $aws->upload(request()->file('file'),"Hsups")['full_url'];*/
        $model->update($data);
    ...
    function generateLink($id,$data) {
        $code = Core::generateUniqueCode(Hsup::class, 10, 'FET');
        $link_token = Str::random(40); // Génère un token de 40 caractères
        $url = env('APP_FRONT_URL').'/Hsup/'.$code.'/'.$link_token;
    ...
    function generateMediaLink($id) {
        ...
        $url = env('APP_FRONT_URL').'/Hsup-gallery/'.$code.'/'.$media_token;
        $url = mb_convert_encoding($url, 'UTF-8', 'auto'); // Force l'encodage en UTF-8
    ...
    function verifyLink($data) {
        if (isset($data['link_token'])) {
            return Hsup::where('link_token', )->first();
        }else{
            return Hsup::where('media_token', $data['media_token'])->first();
    ...
    function participate($data){
        $check=Invite::where('Hsup_id', $data['Hsup_id'])
