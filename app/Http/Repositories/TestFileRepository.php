...
class TestFileRepository
{
    ...
    public function getAll($request)
    {
       $per_page = 10;
        ...
        $req = TestFile::ignoreRequest(['per_page','pageSize','page'])
     ...
     */
    public function makeStore($data): TestFile
    {
           if (request()->hasFile('file')) {
                $data['filename'] = FileStorage::setFile('public', request()->file('file'), 'tests', Str::slug($data['title']).time());
            }

       $model = new TestFile($data);
     ...
     * Met à jour une fête.
     */
    public function makeUpdate($id, $data): TestFile
    {
      $model = TestFile::findOrFail($id);
        /* $aws = new AwsService();
        if (request()->file('file')) {
            $data['file'] = $aws->upload(request()->file('file'), "TestsFiles")['full_url'];
        } */
        $model->update($data);
    ...
    {
        $query = TestFile::query();
        $attrs = ['nom', 'lieu', 'type_TestsFile'];
    ...
    function generateLink($id,$data) {
        $code = Core::generateUniqueCode(TestFile::class, 10, 'FET');
        $link_token = Str::random(40); // Génère un token de 40 caractères
        $url = env('APP_FRONT_URL').'/TestFile/'.$code.'/'.$link_token;
    ...
    function generateMediaLink($id) {
        ...
        $url = env('APP_FRONT_URL').'/TestFile-gallery/'.$code.'/'.$media_token;
        $url = mb_convert_encoding($url, 'UTF-8', 'auto'); // Force l'encodage en UTF-8
    ...
    function verifyLink($data) {
        if (isset($data['link_token'])) {
            return TestFile::where('link_token', $data['link_token'])->first();
        }else{
            return TestFile::where('media_token', $data['media_token'])->first();
    ...
    function participate($data){
        $check=Invite::where('TestsFile_id', $data['TestsFile_id'])
