...
use App\Models\AppelsOffreFile;
...
class AppelsOffreFileRepository
{
    use Repository;
    ...
    {
        $this->model = app(AppelsOffreFile::class);
    }
     ...
     */
    public function makeStore($data): AppelsOffreFile
    {
        $model = new AppelsOffreFile($data);
        /*$aws= new AwsService();
        if(request()->file('file'))  $model->file = $aws->upload(request()->file('file'),"AppelsOffreFiles")['full_url'];*/
        $model->save();
     ...
     * Met à jour une fête.
     */
    public function makeUpdate($id, $data): AppelsOffreFile
    {
        $model = AppelsOffreFile::findOrFail($id);
        /*$aws= new AwsService();
        if(request()->file('file'))  $data['file'] = $aws->upload(request()->file('file'),"AppelsOffreFiles")['full_url'];*/
        $model->update($data);
    ...
    {
        $query = AppelsOffreFile::query();
        $attrs = ['nom', 'lieu', 'type_AppelsOffreFile'];
    ...
    function generateLink($id,$data) {
        $code = Core::generateUniqueCode(AppelsOffreFile::class, 10, 'FET');
        $link_token = Str::random(40); // Génère un token de 40 caractères
        $url = env('APP_FRONT_URL').'/AppelsOffreFile/'.$code.'/'.$link_token;
    ...
    function generateMediaLink($id) {
        ...
        $url = env('APP_FRONT_URL').'/AppelsOffreFile-gallery/'.$code.'/'.$media_token;
        $url = mb_convert_encoding($url, 'UTF-8', 'auto'); // Force l'encodage en UTF-8
    ...
    function participate($data){
        $check=Invite::where('AppelsOffreFile_id', $data['AppelsOffreFile_id'])
