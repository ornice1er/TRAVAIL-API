...
class UserRepository
{
    ...
    public function getAll($request)
    {
        ...
        // Exemple de filtre avancé (commenté)
    ...
    {
        // Gestion du fichier photo/avatar
        ...
        // Récupération et suppression des clés non liées au modèle User
        $roleName = $data['role'] ?? null;
        $projectId = $data['project_id'] ?? null;
        unset($data['role'], $data['project_id']);
        ...
        // Liaison utilisateur - projet si projet renseigné
        if ($projectId) {
            UserProject::create([
                'user_id' => $model->id,
        ...
        // Envoi de mail à l'utilisateur avec ses identifiants
        Mailer::sendSimple(
            'emails.new_account',
...
public function changePassword()
{
    ...
    if (!Hash::check($oldPassword, $user->password)) {
        session()->flash('danger', 'Ancien mot de passe incorrect');
        return back();
    }

    if ($newPassword !== $confirmPassword) {
        session()->flash('info', 'Les nouveaux mots de passe ne correspondent pas');
        return back();
    }
    ...
    public function up($id)
{
    ...
    request()->session()->flash('success', 'Un compte utilisateur activé avec succès');
...
public function down($id)
{
    ...
    request()->session()->flash('success', 'Un compte utilisateur désactivé avec succès');
