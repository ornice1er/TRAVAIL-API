...
class TransmissionRepository
{
    ...
    {
        $this->model = app(Transmission::class);
    }
    ...
    public function getAll($request)
    {
        ...
        $req = Transmission::ignoreRequest(['per_page','pageSize','page'])
     ...
     */
    public function makeStore($data): Transmission
    {
        $model = new Transmission($data);
        /*$aws= new AwsService();
        if(request()->file('file'))  $model->file = $aws->upload(request()->file('file'),"Transmissions")['full_url'];*/
        $model->save();
     ...
     * Met à jour une fête.
     */
    public function makeUpdate($id, $data): Transmission
    {
        $model = Transmission::findOrFail($id);
        /*$aws= new AwsService();
        if(request()->file('file'))  $data['file'] = $aws->upload(request()->file('file'),"Transmissions")['full_url'];*/
        $model->update($data);
    ...
    public function search($term)
    {
        ...
        $attrs = ['nom', 'lieu', 'type_Transmission'];
    ...
    function generateLink($id,$data) {
        $code = Core::generateUniqueCode(Transmission::class, 10, 'FET');
        $link_token = Str::random(40); // Génère un token de 40 caractères
        $url = env('APP_FRONT_URL').'/Transmission/'.$code.'/'.$link_token;
    ...
    function generateMediaLink($id) {
        ...
        $url = env('APP_FRONT_URL').'/Transmission-gallery/'.$code.'/'.$media_token;
        $url = mb_convert_encoding($url, 'UTF-8', 'auto'); // Force l'encodage en UTF-8
    ...
    function verifyLink($data) {
        if (isset($data['link_token'])) {
            return Transmission::where('link_token', )->first();
        }else{
            return Transmission::where('media_token', $data['media_token'])->first();
    ...
    function participate($data){
        $check=Invite::where('Transmission_id', $data['Transmission_id'])
